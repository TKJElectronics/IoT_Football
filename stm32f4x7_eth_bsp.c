...
void ETH_BSP_Config(void)
{
  ...
  Eth_Link_PHYITConfig(Eth_PhyAddr);
...
static void ETH_MACDMA_Config(void)
{
  ...
  for(Eth_PhyAddr = 1; 32 >= Eth_PhyAddr; Eth_PhyAddr++)
  {    
    ...
    PHYID.set.HI = ETH_ReadPHYRegister(Eth_PhyAddr,2);  // 0x0022
    PHYID.set.LO = ETH_ReadPHYRegister(Eth_PhyAddr,3);  // 0x1619
  ...
  /* Configure Ethernet */
  EthInitStatus = ETH_Init(&ETH_InitStructure, Eth_PhyAddr);
